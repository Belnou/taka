...
2 = LIGNE = PATIENT
CREATION DE LA BASE DE DONNEE LARGE
```{r}
...
#date premiere rechute et presence rechute
bdls <- bdls %>% mutate(
...
bdls %>% mutate(a = sum(rechute_1_ligne_1,rechute_1_ligne_2, na.rm = T))

bd_sum <- bdls %>%  select(n, contains("rechute_1_l")) %>% 
  mutate( nb_rechute = sum(2:5, na.rm = TRUE))

# LES VERIFICATIONS
# bdls %>% select(n,presence_rechute, contains("date_r"), date_premiere_rechute)

bdls %>% mutate(nombre_rechute = rechute_1_ligne_1+rechute_1_ligne_2+rechute_1_ligne_3+rechute_1_ligne_4+rechute_1_ligne_5)

# verif
#bdls %>% filter(n==3) %>% select(n, contains("rechute_"))

 # mutate(nombre_rechute = sum(rechute_1_ligne_1,rechute_1_ligne_2,rechute_1_ligne_3,rechute_1_ligne_4,rechute_1_ligne_5))
bdls %>% select(contains("rechute_1_l")) %>% glimpse
