...
bd_traitement %>% select(contains("aspi")) %>% colnames
bd1 %>% select(contains("compl")) %>% colnames
bd1 %>% select(contains("rech")) %>% colnames

bd1 %>% select(contains("aspi"))
```
...
CREATION DE LA BASE de donné complete
```{r rows.print=30}
...
essai <- mabase %>% select(n, num_ligne_ttt, contains("aspir")) %>% filter(presence_aspirine==0) %>% filter(aspirine_end_ttt>0)
suum<- function(x){sum(x, na.rm=TRUE)}
map_dbl(essai, (suum))

rm(essai)

mabase %>% filter(n %in% c("19", "42", "44")) %>% select(n, num_ligne_ttt, contains("aspir")) %>% filter(num_ligne_ttt==3)
```


Prendre un traitement par statine diminu les complications vasculaires
```{r }
verif_complic <- bd_statine %>% select(n, num_ligne_ttt, date_j0_ttt, date_fin_ligne_therapeutique_ttt, complication_1, date_complication_n_1, complication_2, date_complication_n_2) %>% arrange(n, num_ligne_ttt) %>% 
  filter(complication_1==1 | complication_2==1)

write.table(verif_complic,"verif_complic.csv", row.names = FALSE, sep ="*", dec=",", na=" ")
```
...
bd_statine$presence_statine <- as.factor(bd_statine$presence_statine)
baselines <- compareGroups(presence_statine ~ dose_prednisone_end_ttt+rechute_1, data = bd_statine, include.miss = F )
createTable(baselines, show.ratio = FALSE, show.p.overall = FALSE)

names(bd_statine)
bd_statine$presence_statine)

bd_statine %>% 
  group_by(presence_statine) %>% 
  summarise(
    n = n(),
    moyenne = round(mean (dose_prednisone_end_ttt, na.rm=TRUE),4),
    sd = round(sd (dose_prednisone_end_ttt, na.rm=TRUE),4),
  )
